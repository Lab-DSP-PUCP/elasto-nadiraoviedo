...
function [vshears,vshearc]= AMFM_demod_StWS(sono_filt_mov,Properties)
...
%        vshearsin           vshearcos
    ...
    numChannels = 3;                   % Número de canales
    r0 = 0.1;                            % Frecuencia base
    commonRatio = 1.8;                 % Razón geométrica
    octaveBandwidth = 1.8;             % Ancho de banda de 1.5 octavas
    ...
    for slice = 1:size(sono_filt_mov,1)
        ...
        vshearsin_pre(slice,:)=(pi*Properties.f_vib)./(grad_phi_sin*10^3)*1.2;
        vshearcos_pre(slice,:)=(pi*Properties.f_vib)./(grad_phi_cos*10^3);
